...
- Bind the command/query **directly** as the request body - `Create([FromBody] CreateLoadCommand request)`. The command/query record _is_ the wire contract; there is no `*Dto` wrapper on the write side.
