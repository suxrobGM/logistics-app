...
- **Always prefer a shared `ui-*` component from `@logistics/shared/ui` over hand-rolled Tailwind.** The Helm primitives under `ui/primitives/` are an implementation detail and are deliberately **not** re-exported - never reach for a raw `hlm*` directive in feature code. The full public catalogue:
  ...
  - **Menus**: `ui-menu` - a popup menu driven by a template ref: `<ui-menu #menu [items]="items()" />` plus `(click)="menu.toggle($event)"` on any trigger. Items are `UiMenuItem` (`icon` is a typed `IconName`; `variant: "destructive"` for danger rows; `visible: false` **removes** an item). Use it for any list of actions - `ui-popover` gives you no arrow keys and no `role="menu"`. An optional `[menuHeader]` child renders above the items; it must be a **single static root node**, since an `@if` at the root of projected content never matches a `select=` slot
