 ...
 * This function retrieves the token from the session storage key "0-logistics.officeapp".
 * @returns The authentication access token or null if not found.
 */
export function getAccessToken(): string | null {
  const authData = JSON.parse(sessionStorage.getItem("0-logistics.officeapp") || "{}");
