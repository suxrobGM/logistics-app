    ...
    Column(modifier = modifier) {
        ...
        Canvas(
            modifier = Modifier
                ...
                .pointerInput(data) {
                    detectTapGestures { offset ->
                        ...
                        selectedIndex =
                            if (selectedIndex == tappedIndex) null else tappedIndex.coerceIn(data.indices)
                    }
                }
        ) {
            ...
            data.forEachIndexed { index, item ->
                ...
                // Primary bar (Gross)
                val primaryBarHeight =
                    ((item.gross / maxValue) * chartHeight * animationProgress.value).toFloat()
                drawRoundRect(
                ...
                if (config.showSecondaryBars) {
                    val secondaryBarHeight =
                        ((item.driverShare / maxValue) * chartHeight * animationProgress.value).toFloat()
                    drawRoundRect(
                        color = if (selectedIndex == index) secondaryColor.copy(alpha = 0.8f) else secondaryColor,
                        topLeft = Offset(
                            groupStartX + barWidth + 4f,
