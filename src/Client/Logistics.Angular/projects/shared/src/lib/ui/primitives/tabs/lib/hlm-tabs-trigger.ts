 ...
 *
 * `cursor-pointer select-none` is ours too: upstream puts the trigger on a `<button>`, which gets
 * both from the browser. Here it sits on `ui-tab`'s host element, so text hover shows an I-beam and
 * the label is selectable unless we say otherwise.
 *
