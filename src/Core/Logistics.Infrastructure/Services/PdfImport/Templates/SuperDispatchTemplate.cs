...
public sealed partial class SuperDispatchTemplate : IDispatchSheetTemplate
{
    ...
    [GeneratedRegex(@"(\d+[^,]*?(?:Rd|Road|St|Street|Ave|Avenue|Blvd|Boulevard|Dr|Drive|Ln|Lane|Way|Ct|Court|Pl|Place|Cir|Circle)[^,]*?),?\s*([A-Za-z\s]+),\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)", RegexOptions.IgnoreCase)]
