...
public class MessagingHubContext
{
    ...
    public Guid? GetUserId(string connectionId) =>
        connectedClients.TryGetValue(connectionId, out var connection) ? connection.UserId : null;

    /// <summary>
    ///     Gets the tenant ID for a connection.
    /// </summary>
    public string? GetTenantId(string connectionId) =>
        connectedClients.TryGetValue(connectionId, out var connection) ? connection.TenantId : null;
    ...
    /// </summary>
    public int GetConnectionCount() => connectedClients.Count;
