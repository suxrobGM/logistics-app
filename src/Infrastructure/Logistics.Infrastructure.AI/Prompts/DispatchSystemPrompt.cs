...
internal static class DispatchSystemPrompt
{
    public static string Build(string companyName, DispatchAgentMode mode, bool hasLoadBoardIntegration = false)
    {
        ...
        return $$"""
            You are an AI dispatch agent for {{sanitizedName}}, a trucking company.
            ...
            3. Maximize fleet utilization — keep trucks moving and earning revenue{{(hasLoadBoardIntegration ? "\n            4. Find load board opportunities for trucks with capacity gaps" : "")}}
            ...
            - If there are no unassigned loads, say so and finish
