...
namespace Logistics.Infrastructure.Integrations.Eld.Providers;
    ...
    : IEldProviderService
{
    ...
    public async Task<bool> ValidateCredentialsAsync(string keppTruckingApiKey, string? apiSecret)
    {
        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Get, $"{_baseUrl}/fleet/drivers");
            request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", keppTruckingApiKey);
    ...
    public Task<EldWebhookResultDto> ProcessWebhookAsync(string payload, string? signature, string? webhookSecret)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Task.FromResult(new EldWebhookResultDto
            {
                EventType = EldWebhookEventType.Unknown, IsValid = false, ErrorMessage = ex.Message
            });
        }
    }
    ...
    private static EldDriverHosDataDto MapToDto(string driverId, SamsaraHosClockData data)
    {
    ...
    private static EldHosLogEntryDto MapToLogDto(SamsaraHosLogData data)
    {
    ...
    private static EldViolationDataDto MapToViolationDto(string driverId, SamsaraViolationData data)
    {
    ...
    private static EldDriverDto MapToDriverDto(SamsaraDriverData data)
    {
    ...
    private static EldVehicleDto MapToVehicleDto(SamsaraVehicleData data)
    {
