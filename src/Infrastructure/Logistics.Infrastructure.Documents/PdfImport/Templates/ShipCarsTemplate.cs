...
/// </summary>
internal sealed partial class ShipCarsTemplate : IDispatchSheetTemplate
{
    ...
    private static Address? ExtractDestinationAddress(string text)
    {
        ...
        if (vehicleIdx > 0 && vehicleIdx < endIdx)
        {
            endIdx = vehicleIdx;
        }

        if (vipIdx > 0 && vipIdx < endIdx)
        {
            endIdx = vipIdx;
        }
    ...
    [GeneratedRegex(
        @"(\d+[^,]*?(?:Ave|Avenue|St|Street|Rd|Road|Blvd|Boulevard|Dr|Drive|Ln|Lane|Way|Ct|Court|Pl|Place|Cir|Circle|Trail)[^,]*?),?\s*([A-Za-z\s]+),?\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)",
