...
internal sealed class MapboxMatrixTripOptimizer(MapboxMatrixClient matrix) : ITripOptimizer
{
        ...
        CancellationToken ct = default)
    {
        ...
        // 2) Query Matrix
        var matrix1 = await matrix.GetMatrixAsync(coords, ct);
        ...
        while (remaining.Count > 0)
        {
            ...
            foreach (var s in remaining.Values)
            {
                ...
                var cost = matrix1.Cost(curIdx, toIdx);
                if (cost < bestCost)
                {
                    bestCost = cost;
                    best = s;
