...
public class AIDispatchPolicyLearnerTests
{
    ...
    private readonly AIDispatchPolicyLearner _sut;
    ...
    public AIDispatchPolicyLearnerTests()
    {
        ...
        _sut = new AIDispatchPolicyLearner(
            _tenantUow, _llmClient, llmOptions, NullLogger<AIDispatchPolicyLearner>.Instance);
    }
    ...
    public async Task Learn_TooFewDecisions_SkipsWithoutCallingLlm()
    {
        ...
        var result = await _sut.LearnForCurrentTenantAsync();

        Assert.True(result.IsSuccess);
        Assert.False(result.Value!.Generated);
        Assert.Contains("Not enough reviewed decisions", result.Value.SkipReason);
        await _llmClient.DidNotReceiveWithAnyArgs().CompleteAsync(default!, default);
    }
    ...
    public async Task Learn_ForcedButTooFewDecisions_StillSkips()
    {
        ...
        var result = await _sut.LearnForCurrentTenantAsync(force: true);
    ...
    public async Task Learn_ExcludesUnapprovedExecutionsAndQueries()
    {
        ...
        await _sut.LearnForCurrentTenantAsync();
    ...
    {
        _decisionRepo.Query().Returns((decisions ?? []).BuildMock());
    }
    ...
    {
        return (LlmCompletionRequest)_llmClient.ReceivedCalls()
