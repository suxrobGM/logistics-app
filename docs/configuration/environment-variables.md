...
| `Presets`          | no       | Tenant presets, e.g. `["GeneralFreight", "SoloOperator"]`. Empty means `GeneralFreight`. When the list changes, the next run re-applies the preset features.                                                                         |
...
```bash
License__Key=""
License__HeartbeatEnabled=true
